...
### 阶段 1 · 体检定位（确定性）
跑 `chinese-geo audit <域名> --format json`，拿到 `score` / `band` / `recommendations`。委派 **`chinese-geo-audit`** 出中文行动清单。**先看有没有 Critical（尤其国内爬虫被挡）——有就先修，别的先放。**
...
### 阶段 6 · 监控验证（委派 `chinese-geo-monitor`）
