# 在 Trae（字节）里用 chinese-geo

> chinese-geo（命令 `chinese-geo`）：让网站被国内（豆包 / DeepSeek / 文心 / 通义 / 元宝 / Kimi）与
> 海外（ChatGPT / Claude / Perplexity / Google AI）AI 引擎抓取与引用。本卡＝在 Trae 里的装法 + 调法 + 真实输出。
...
## 2. 接入 Trae
```bash
chinese-geo init --agent trae
```
写入（已存在则跳过，不覆盖）：
- `.trae/rules/project_rules.md` —— Trae 的项目规则（指令）文件
- `.trae/mcp.json` —— Trae 项目级 MCP 配置（键 `mcpServers`）

> 置信：指令落点高；MCP 路径（`.trae/mcp.json`）以 Trae 官方文档为准（中等置信，版本间可能变）。

## 3. 调法
- **CLI 直调**（永远可用）：让 Trae 跑 `chinese-geo audit <你的域名>`。
- **MCP 工具**：`.trae/mcp.json` 就位后，`chinese-geo` 的 8 个工具可调。
- **Skills**：放进 `.trae/skills/<name>/SKILL.md`。
