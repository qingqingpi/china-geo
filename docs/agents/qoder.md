# 在 Qoder（阿里）里用 chinese-geo

> chinese-geo（命令 `chinese-geo`）：让网站被国内（豆包 / DeepSeek / 文心 / 通义 / 元宝 / Kimi）与
> 海外（ChatGPT / Claude / Perplexity / Google AI）AI 引擎抓取与引用。本卡＝在 Qoder 里的装法 + 调法 + 真实输出。
...
## 2. 接入 Qoder
```bash
chinese-geo init --agent qoder
```
写入（已存在则跳过，不覆盖）：
- `AGENTS.md` —— Qoder 原生读根 `AGENTS.md`，写它即可（省去 `.qoder/rules/` 一套）
- `MCP-SETUP-qoder.md` —— **手动一步**：Qoder 的 MCP 是 UI-only（设置面板里配，无稳定项目级文件），
  不替你写它的私有路径；打开 Qoder 的 MCP / 工具设置，把文件里的 JSON 贴进去

> 置信：指令落点高；MCP 路径以 Qoder 官方设置面板为准（社区/官方文档可能随版本变）。

## 3. 调法
- **CLI 直调**（永远可用）：让 Qoder 跑 `chinese-geo audit <你的域名>`。
- **MCP 工具**：按 `MCP-SETUP-qoder.md` 在面板配好后，`chinese-geo` 的 8 个工具可调。
- **Skills**：放进 `.qoder/skills/<name>/SKILL.md`。
