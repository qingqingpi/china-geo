# 在 Codex 里用 chinese-geo

> chinese-geo（命令 `chinese-geo`）：让网站被国内（豆包 / DeepSeek / 文心 / 通义 / 元宝 / Kimi）与
> 海外（ChatGPT / Claude / Perplexity / Google AI）AI 引擎抓取与引用。本卡＝在 Codex 里的装法 + 调法 + 真实输出。
...
## 2. 接入 Codex
```bash
chinese-geo init --agent codex
```
写入（已存在则跳过，不覆盖）：
- `AGENTS.md` —— 事实标准指令文件，Codex 自动读到
- `.mcp.json` —— `chinese-geo` MCP 服务声明

Codex 的 MCP 走全局 `~/.codex/config.toml`，把 `.mcp.json` 里的服务并过去：
```toml
...
## 3. 调法
- **CLI 直调**（永远可用）：让 Codex 跑 `chinese-geo audit <你的域名>`。
- **MCP 工具**：配好 config.toml 后，`chinese-geo` 的 8 个工具可直接调。
- **Skills**：Codex 支持 Agent Skills，把 `skills/*/SKILL.md`（vendor-neutral）放进 Codex 的 skills 目录即可。
